...
<body class="sb-nav-fixed">
    ...
    <div id="layoutSidenav">
        ...
        <div id="layoutSidenav_content">
            <main class="container mt-4">
                ...
                <h1 class="text-center mb-4">Attendance Report for {{ date('F', mktime(0, 0, 0, $month, 1)) }} {{ $year }}</h1>
                <!-- Bar Chart -->
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <span>Attendance Report</span>
                        <!-- 筛选月份和年份 -->
                        <form method="GET" action="{{ route('admin.reports') }}" class="d-flex">
                        ...
                        <table class="table table-bordered table-hover" id="attendanceTable">
                            ...
                            <tbody>
                                @foreach($staffNames as $index => $name)
                                    <tr>
                                        <td>
                                            <a href="{{ route('admin.viewStaffAttendance', $staffIds[$index]) }}">
                                                {{ $name }}
                                            </a>
                                        </td>
                                        <td>{{ $staffEmails[$index] }}</td>
                                        <td>{{ $attendanceDays[$index] }}</td>
                                        <td>{{ $absenceDays[$index] }}</td>
                                        <td>{{ $leaveDays[$index] }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const labels = @json($staffNames);
            const attendanceDays = @json($attendanceDays);
            const leaveDays = @json($leaveDays);
            const absenceDays = @json($absenceDays);
            const daysUntilToday = @json($daysUntilToday); // 当前日期之前的天数
